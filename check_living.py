...
# fig = px.scatter(x=planet_radius, y=planet_mass,color=planetTypeValues)
# fig.show()
...
for index,data in enumerate(planetData):
    ...
    #goldiLock
    try:
        if float(data[8].split(" ")[0]) > 0.38 and float(data[8].split(" ")[0]) < 2:
            features.append("goldiLock")
    except:
        try:
            if data[8]>0.38 and data[8]<2:
                features.append("goldiLock")
        except:
            pass
    ...
    try:
        try:
            ...
            try:
                distance = 2 * 3.14 * (float(data[8]) * 1.496e+9)
            except:
                pass
            try:
        ...
        pass
    finalDic[data[1]]=features



# goldiLock(no. of planets)
goldi_Lock_Planets = 0
for key,value in finalDic.items():
    if "goldiLock"in value:
        goldi_Lock_Planets = goldi_Lock_Planets + 1

# Speeed(no. of planets)
speeed_Planets = 0
for key,value in finalDic.items():
    if "Speeed"in value:
        speeed_Planets = speeed_Planets + 1

# Gravity(no. of planets)
gravity_planets = 0
for key,value in finalDic.items():
    if "gravity"in value:
        gravity_planets = gravity_planets + 1

# Planet_Type(no. of planets)
planet_type_planets = 0
for key,value in finalDic.items():
    if "planet_type" in value:
        planet_type_planets = planet_type_planets + 1

gravity_planets_not = []
for data in planetData:
    if data not in low_gravity_planets:
        gravity_planets_not.append(data)

no_gravity_count = 0
for data in gravity_planets_not:
    if data[6].lower() == "terrestrial" or data[6].lower() == "super earth":
        no_gravity_count = no_gravity_count + 1


# No. of habitable planets
Living_planets = 0
for key,value in finalDic.items():
    if "planet_type" in value and "goldiLock" in value and "gravity" in value and "Speeed" in value:
        Living_planets = Living_planets + 1
print(Living_planets)